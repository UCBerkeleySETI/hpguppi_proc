...
__global__
void data_transpose(signed char* data_in, cuComplex* data_tra, int offset, int n_pol, int n_samp, int n_chan);

// Perform beamforming operation
__global__
void coherent_beamformer(cuComplex* input_data, float* coeff, float* output_data, int offset, int n_pol, int n_chan, int n_beam);

// Compute power of beamformer output (abs()^2)
__global__
void beamformer_power(float* bf_volt, float* bf_power, int offset, int n_pol, int n_beam, int n_chan, int n_samp);
...
void data_transpose(signed char* data_in, cuComplex* data_tra, int offset, int n_pol, int n_samp, int n_chan) {
	...
	if(f < n_freq_streams){
		int h_in = data_in_idx(p, t, (f + offset), a, n_pol, n_samp, n_chan); // (p, t, f, a, Np, Nt, Nf) 
		int h_tr = data_tr_idx(a, p, (f + offset), t, n_pol, n_chan); // (a, p, f, t, Np, Nf)
...
void coherent_beamformer(cuComplex* input_data, float* coeff, float* output_data, int offset, int n_pol, int n_chan, int n_beam) {
	...
	if(f < n_freq_streams){
		for (int p = 0; p < n_pol; p++) { // Polarization index
			...
			int i = data_tr_idx(a, p, (f + offset), t, n_pol, n_chan); // (a, p, f, t, Np, Nf)
...
void beamformer_power(float* bf_volt, float* bf_power, int offset, int n_pol, int n_beam, int n_chan, int n_samp) {
	...
	if(f < n_freq_streams){	
			...
			int xp = coh_bf_idx(0, b, (f + offset), t, n_pol, n_beam, n_chan); // X polarization
			float x_pol_pow = (bf_volt[2 * xp] * bf_volt[2 * xp]) + (bf_volt[2 * xp + 1] * bf_volt[2 * xp + 1]); // XX*

			int h = pow_bf_nosti_idx(b, (f + offset), t, n_chan, n_samp);
			bf_power[h] = x_pol_pow; // XX*
		}else if(n_pol == 2){
			// Power = Absolute value squared of output -> r^2 + i^2
			int xp = coh_bf_idx(0, b, (f + offset), t, n_pol, n_beam, n_chan); // X polarization
			int yp = coh_bf_idx(1, b, (f + offset), t, n_pol, n_beam, n_chan); // Y polarization
			float x_pol_pow = (bf_volt[2 * xp] * bf_volt[2 * xp]) + (bf_volt[2 * xp + 1] * bf_volt[2 * xp + 1]); // XX*
...
// Run beamformer
float* run_beamformer(signed char* data_in, float* h_coefficient, int n_pol, int n_beam, int n_chan, int n_samp) {

	cudaError_t err_code;

	//const int freq_chans = N_FREQ_STREAM;
	int n_freq_streams = n_chan/N_STREAMS;
        int n_win = n_samp/N_TIME_STI;

	// Transpose kernel: Specify grid and block dimensions
	dim3 dimBlock_transpose(N_ANT, n_pol, 1);
	dim3 dimGrid_transpose(n_samp, n_chan, 1);
	...
	//dim3 dimGrid_bf_pow(n_samp, n_chan, 1);
	...
	const int streamSizeIn = (2*N_ANT*n_pol*n_samp*n_freq_streams);
	const unsigned long int streamBytesIn = (2*N_ANT*n_pol*n_samp*n_freq_streams*sizeof(signed char));

	// beamformer_power kernel and cudaMemcpy(DtoH)
	//const int streamSizePow = (n_beam*n_samp*n_freq_streams);
	//const unsigned long int streamBytesPow = (n_beam*n_samp*n_freq_streams*sizeof(float));
	const int streamSizePow = (n_beam*n_win*n_freq_streams);
	const unsigned long int streamBytesPow = (n_beam*n_win*n_freq_streams*sizeof(float));
	...
	for (int i = 0; i < N_STREAMS; ++i){
		int offset = i * n_freq_streams;
		// Perform transpose on the data and convert to floats  
		data_transpose<<<dimGrid_transpose, dimBlock_transpose, 0, stream[i]>>>(d_data_in, d_data_tra, offset, n_pol, n_samp, n_chan);
	...
	for (int i = 0; i < N_STREAMS; ++i){
		...
		coherent_beamformer<<<dimGrid_coh_bf, dimBlock_coh_bf, 0, stream[i]>>>(d_data_tra, d_coefficient, d_data_bf, offset, n_pol, n_chan, n_beam);
		err_code = cudaGetLastError();
		if (err_code != cudaSuccess) {
	...
	*/
			...
			for (int t = 0; t < n_samp; t++) {
				for (int f = 0; f < n_chan; f++) {
					for (int a = 0; a < N_ANT; a++) {
						...
						tmp = (tmp + 1) % (N_ANT+1);
						if(a < N_REAL_ANT){
							data_sim[2 * data_in_idx(p, t, f, a, n_pol, n_samp, n_chan)] = tmp;
						}else{
							data_sim[2 * data_in_idx(p, t, f, a, n_pol, n_samp, n_chan)] = 0;
						}
					}
				}
			}
		...
		for (int p = 0; p < n_pol; p++) {
			for (int t = 0; t < n_samp; t++) {
				for (int a = 0; a < N_ANT; a++) {
					...
					tmp = (tmp + 1) % (N_ANT+1);
					if(a < N_REAL_ANT){
						data_sim[2 * data_in_idx(p, t, 5, a, n_pol, n_samp, n_chan)] = tmp;
						data_sim[2 * data_in_idx(p, t, 2, a, n_pol, n_samp, n_chan)] = tmp;
					}else{
						data_sim[2 * data_in_idx(p, t, 5, a, n_pol, n_samp, n_chan)] = 0;
						data_sim[2 * data_in_idx(p, t, 2, a, n_pol, n_samp, n_chan)] = 0;
					}
				}
			}
		}
	...
	if (sim_flag == 3) {
		...
		for (int t = 0; t < n_samp; t++) {
			...
			theta = ((t/50 - (n_samp / 100)) + cb)*PI/180; // SOI direction/angle of arrival -> Moving across array over time i.e. angle changes each time sample
				...
				for (int a = 0; a < N_ANT; a++) {
					if(n_pol == 1){
						if(a < N_REAL_ANT){
							...
							// X polarization
							data_sim[2 * data_in_idx(0, t, f, a, n_pol, n_samp, n_chan)] = (signed char)((((cos(2 * PI * rf_freqs * a * tau) - tmp_min)/(tmp_max-tmp_min)) - 0.5)*256);
							data_sim[2 * data_in_idx(0, t, f, a, n_pol, n_samp, n_chan) + 1] = (signed char)((((sin(2 * PI * rf_freqs * a * tau) - tmp_min)/(tmp_max-tmp_min)) - 0.5)*256);
						}else{
							// X polarization
							data_sim[2 * data_in_idx(0, t, f, a, n_pol, n_samp, n_chan)] = 0;
							data_sim[2 * data_in_idx(0, t, f, a, n_pol, n_samp, n_chan) + 1] = 0;
						}
					}else if(n_pol == 2){
						if(a < N_REAL_ANT){
							// Requantize from doubles/floats to signed chars with a range from -128 to 127
							...
							data_sim[2 * data_in_idx(0, t, f, a, n_pol, n_samp, n_chan) + 1] = (signed char)((((sin(2 * PI * rf_freqs * a * tau) - tmp_min)/(tmp_max-tmp_min)) - 0.5)*256);
							// Y polarization
							data_sim[2 * data_in_idx(1, t, f, a, n_pol, n_samp, n_chan)] = (signed char)((((cos(2 * PI * rf_freqs * a * tau) - tmp_min)/(tmp_max-tmp_min)) - 0.5)*256);
							data_sim[2 * data_in_idx(1, t, f, a, n_pol, n_samp, n_chan) + 1] = (signed char)((((sin(2 * PI * rf_freqs * a * tau) - tmp_min)/(tmp_max-tmp_min)) - 0.5)*256); // Make this negative if a different polarization is tested
						}else{
							// X polarization
							data_sim[2 * data_in_idx(0, t, f, a, n_pol, n_samp, n_chan)] = 0;
							data_sim[2 * data_in_idx(0, t, f, a, n_pol, n_samp, n_chan) + 1] = 0;
							// Y polarization
							data_sim[2 * data_in_idx(1, t, f, a, n_pol, n_samp, n_chan)] = 0;
							data_sim[2 * data_in_idx(1, t, f, a, n_pol, n_samp, n_chan) + 1] = 0; // Make this negative if a different polarization is tested
						}
					}
				}
			}
			...
			for (int b = 0; b < n_beam; b++) {
				for (int a = 0; a < N_ANT; a++) {
					if(a < n_real_ant){
						...
						coefficients[2 * coeff_idx(a, p, b, f, n_pol, n_beam)] = cos(2 * PI * coarse_chan[f] * (t*delay_rate + delay_offset));
...
int main() {
	...
	// Generate simulated data
	signed char* sim_data = simulate_data(n_pol, n_chan, n_samp);
	...
	for(int ii = 0; ii < num_runs; ii++){
		...
		// Run beamformer 
		output_data = run_beamformer(sim_data, sim_coefficients, n_pol, n_beam, n_chan, n_samp);
