...
static int get_header_size(int fdin, char *header_buf, size_t len)
{
    ...
    if (rv == -1)
    {
        hashpipe_error("hpguppi_stride_input_thread", "error reading file when getting header size");
    }
    else if (rv > 0)
    {
...
static void *run(hashpipe_thread_args_t *args)
{
    ...
    int n_subband = 128;       // Number of sub bands processed serially
                ...
                else if (((raw_file_size - cur_pos) < (blocsize + headersize)))
                {
                    ...
                    else
                    {
                        ...
                        if (s < n_subband - 1)
                        {
                            ...
                            // If the RAW files are deleted before processing is over,
      ...
      // Initialize block
                ...
                else if ((cur_pos + (blocsize + headersize)) > raw_file_size)
                {
                    ...
                    if (cur_pos < (raw_file_size - headersize))
                    {
                        ...
                        n_coarse_proc = n_coarse / n_subband;
                        n_samp_per_block = (int)(blocsize / (2 * obsnchan * npol));
                        nblocks = 32; // Set to default since there isn't a precise way to calculate the number of blocks
