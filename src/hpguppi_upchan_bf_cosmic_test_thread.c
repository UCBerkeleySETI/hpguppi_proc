...
static void *run(hashpipe_thread_args_t *args)
{
  ...
  // Loop
  int64_t pktidx = 0, pktstart = 0, pktstop = 507905;
  ...
  int n_subband = 128; // Number of subbands
  ...
  */
  ...
  while (run_threads())
  {
    ...
    hgetr8(p_header, "SYNCTIME", &synctime);
    ...
    strcpy(raw_obsid, "synthetic_test_rand");
    ...
    if (strcmp(prev_basefilename, raw_basefilename) != 0 || subband_idx != prev_subband_idx)
    {
      ...
      {
        coarse_chan_freq[i] = (((i + subband_idx * n_coarse_proc) - (((double)n_chan_per_node - 1) / 2)) * chan_bw + obsfreq) * 1e-3;
        // Equivalent equation //
        // coarse_chan_freq[i] = (((i + subband_idx*n_coarse_proc)-((double)n_chan_per_node/2))*chan_bw + obsfreq + (chan_bw/2))*1e-3;
      }

      // Number of FFT points depending on mode
      if (n_coarse_proc == 1)
      {                 // 1k mode
        n_fft = 1024; //524288; // 2^19 point FFT
      }
      ...
      {
        printf("actual_nbeams = %d, nbeams = %ld, Beam Index = %d\n", actual_nbeams, nbeams, b);
        if (sim_flag == 0)
        {
          // Set specified path to write filterbank files
          if (b == actual_nbeams)
      ...
      */
      ...
      fb_hdr.fch1 = fch1_sb;                         // Center frequency in MHz of the first coarse channel in the subband
    ...
    printf("npol = %d, nants = %d, nbeams = %d, n_coarse_proc = %d, n_win = %d, n_time_int = %d, n_fft = %d, telescope_flag = %d\n", (int)npol, (int)nants, (int)nbeams, (int)n_coarse_proc, n_win, n_time_int, n_fft, telescope_flag);
